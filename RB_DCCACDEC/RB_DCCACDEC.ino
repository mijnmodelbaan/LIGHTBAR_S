...
typedef struct {
   ...
   byte              onoff1;           // Internal use. Output state of accessory: 1=on, 0=off
...
void ConfigureDecoderFunctions() // The amount of accessories must be same as in line 26 above!
{
   ...
   accessory[0].outputPin1  =    1; // Arduino pin to which this accessory is connected
   accessory[0].outputPin2  =    0; // Arduino pin to 2nd pin is for AlternatingFlasher
   accessory[0].ontime      =    0;
   accessory[0].offtime     =    0;

   accessory[1].address     =    2;
   accessory[1].mode        =    2; // Oneshot: HIGH for ontime ms, then LOW and stays LOW.
   accessory[1].outputPin1  =    1;
   accessory[1].outputPin2  =    0;
   accessory[1].ontime      = 1000;
   accessory[1].offtime     =    0;

   accessory[2].address     =    3;
   accessory[2].mode        =    3; // Flasher: HIGH for ontime ms, LOW for offtime ms, repeats till DCC off
   accessory[2].outputPin1  =    0;
   ...
   accessory[3].outputPin1  =    3; // Green signal
   accessory[3].outputPin2  =    4; // Red Signal
   accessory[3].ontime      =    0;
   accessory[3].offtime     =    0;
...
void loop()
{
   ...
   if (accessory[addr].dccstate)
   {
      ...
      {
      case 1: // Continuous
         accessory[addr].onoff1 = HIGH;
         break;

      case 2: // Oneshot
         if (!accessory[addr].onoff1 && !accessory[addr].finished)
         {
            accessory[addr].onoff1 = HIGH;
            accessory[addr].offMilli = millis() + accessory[addr].ontime;
         }
         if (accessory[addr].onoff1 && millis() > accessory[addr].offMilli)
         {
            accessory[addr].onoff1 = LOW;
      ...
      case 3: // Flasher, is an 'alternating' flasher together with outputPin2
         if (!accessory[addr].onoff1 && millis() > accessory[addr].onMilli)
         {
            accessory[addr].onoff1 = 1;
            accessory[addr].onoff2 = 0;
            accessory[addr].offMilli = millis() + accessory[addr].ontime;
         }
         if (accessory[addr].onoff1 && millis() > accessory[addr].offMilli)
         {
            accessory[addr].onoff1 = 0;
   ...
   if (accessory[addr].onoff1) digitalWrite(accessory[addr].outputPin1, LOW);
   else digitalWrite(accessory[addr].outputPin1, HIGH);
